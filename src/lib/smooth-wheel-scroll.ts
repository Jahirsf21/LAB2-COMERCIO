/** Smooth vertical wheel input without restarting an animation for every tick. */
export function smoothWheelScroll(viewport: HTMLElement) {
  ...
  function animate(time: number) {
    ...
    // Use elapsed time so the feel is consistent across different refresh rates.
  ...
  function onWheel(event: WheelEvent) {
    ...
    // A reversal should respond immediately instead of finishing the old movement.
  ...
  function onScroll() {
    // Yield to programmatic navigation and other native scrolling mechanisms.
